...
public static void DesDifferentialAttack3Round() {
	    ...
	    for (BitSet[] pair : pairs) {
		    ...
		    for (int i = 0; i < 64; i++) {
		        ...
		        boolean lhs = (delta_U3R.get(2)) ^ (!delta_U3R.get(7)) ^ (!delta_U3R.get(13)) ^ (!delta_U3R.get(24));
...
public static void DesDifferentialAttack4Round() {
	    ...
	    for (BitSet[] pair : pairs) {
		    ...
		    for (int i = 0; i < 64; i++) {
		    	...
		    	BitSet partialSubkeyGuess = Util.toBitSet(i, 6);
		    	// pad it 
				BitSet subKeyGuess = Util.concatenate(partialSubkeyGuess, 6, Util.toBitSet(0, 24), 24);
		    	subKeyGuess = Util.concatenate(subKeyGuess, 30, Util.toBitSet(0, 18), 18);
		    	...
		    	boolean lhs = (delta_U4R.get(16)) ^ (delta_U4R.get(30)) ^ (!delta_U4R.get(8)) ^ (!delta_U4R.get(22));
