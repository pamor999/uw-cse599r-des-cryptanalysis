 ...
 */
  ...
  public static void MatsuiEqn5Distribution() {
	  ...
	  for (int i = 0; i < numXs; i++) {
	      ...
	      boolean lhs = X.get(31 - 15) ^ F.get(31 - 7) ^ F.get(31 - 18) ^ F.get(31 - 24) ^ F.get(31 - 29);
	      if (lhs == false) count++;
	  }
...
//      // expand
//      BitSet ER = des.E(R);
//      BitSet ERguess = des.E(CR);
//      
//      // XOR in the key
//      ER.xor(K);
//      ERguess.xor(Kguess);
//      
//      System.out.println("ER, and ER guess");
//      printBitSet(ER, 32, 32);
//      printBitSet(ERguess, 32, 32);
//      
//      // Compute S-box values
//      BitSet C = des.SBox(ER);
//      BitSet Cguess = des.SBox(ERguess);
  ...
  public static void DesLinearAttack4Round() {
    ...
    for (BitSet[] pair : pairs) {
	    ...
	    for (int i = 0; i < 64; i++) {
	    	...
	    	// pad it 
			BitSet subKeyGuess = Util.concatenate(partialSubkeyGuess, 6, 
					Util.toBitSet(0, 24), 24);
	    	subKeyGuess = Util.concatenate(subKeyGuess, 30, 
	    			Util.toBitSet(0, 18), 18);
	    	...
	    	//  PH[7,18,25,29] + CH[7,18,24,29] + PL[15] + CL[15] = K1[22] + K3[22] (=0)
	        boolean lhs = PH.get(31 - 7) ^ PH.get(31 - 18) ^ PH.get(31 - 24) ^ PH.get(31 - 29) ^
			  CH_prev.get(31 - 7) ^ CH_prev.get(31 - 18) ^ CH_prev.get(31 - 24) ^ CH_prev.get(31 - 29) ^
			  PL.get(31 - 15) ^ CL_prev.get(31 - 15);
            
            if (lhs == false) counts[i]++;	     
	    }
    }
